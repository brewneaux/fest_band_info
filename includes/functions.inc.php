<?PHP
    ...
    function letterGrouper($urlstring) {
        $letter_groups = array(
            "7a",
            "b",
            "c",
        ...
        $letter_string_links = '<select id="alpha_dropdown" class="alphadropdown__select alphadropdown__select--white" name="URL" onchange="alphadropdownGo()">';
        $letter_string_links .= '<option>sort by letter</option>';
        $letter_string_links .= '<option value=index.php>ALL</option>';
        foreach ($letter_groups as $group) {
            if (strpos($group, $urlstring) == false) {
                if (strlen($group) > 1) {
                    $comma_string = implode(', ',str_split($group)); 
                    $letter_string_links .= '<option value="index.php?letter=' . $group . '">' . $comma_string . '</option>';    
                }
                else {
                    $letter_string_links .= '<option value="index.php?letter=' . $group . '">' . $group . '</option>';
                }
            }
            else {
                $letter_string_links .= $group;
            }
        }
        $letter_string_links .= '</select>';    
        return $letter_string_links;
    }

    function buildIndexQuery($urlstring) {
        if (ctype_alpha($urlstring)) {
            if (strlen($urlstring) == 1) {
                ...
                $string_length = strlen($urlstring);
                $querystring = '';
                for ($i = 0; $i < $string_length; $i++) {
                    if ($i == ($string_length - 1)) {
                        $querystring .= substr($urlstring, $i, 1);
                    }
                    else {
                        $querystring .= substr($urlstring, $i, 1) . '|';
                    }
                ...
                $sql = "SELECT id,band,genre,spotify_uri,spotify_web,bandcamp_offsite,pathtoimage FROM fest_info_working_1 WHERE LEFT(band, 1) REGEXP {$querystring} ORDER BY id ASC";
                return $sql;
            }
        }

        else {
            $sql = "SELECT id,band,genre,spotify_uri,spotify_web,bandcamp_offsite,pathtoimage FROM fest_info_working_1 ORDER BY id ASC";
            return $sql;
        }
    }

    function buildGenreQuery($urlstring) {
        // get array of possible genres
        $possibilities_array = festInfo::genreArray();

        // Santize the input - like, srsly, if they don't put something thats in the possiblity of genres, what are they doing.
        if (in_array($urlstring, $possibilities_array)){
            $genre = trim($urlstring);
            $sql = "SELECT id,band,genre,spotify_uri,spotify_web,bandcamp_offsite,pathtoimage FROM fest_info_working_1 WHERE genre = '{$genre}' ORDER BY id ASC";
            return $sql;    
        }
        else {
            $sql = "SELECT id,band,genre,spotify_uri,spotify_web,bandcamp_offsite,pathtoimage FROM fest_info_working_1 ORDER BY id ASC";
